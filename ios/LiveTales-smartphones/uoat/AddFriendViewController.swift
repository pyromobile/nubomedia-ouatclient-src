    ...
    */
    ...
    @IBAction func sendRequest(sender: UIButton)
    {
        ...
        if( self.friendsSecrectCodeText.text?.isEmpty == false && self.friendsNickText.text?.isEmpty == false )
        {
            if( self.friendsSecrectCodeText.text != self.user.secretCode )
            {
                if( NetworkWatcher.internetAvailabe )
                {
                    ...
                    NotificationModel.sendFriendshipRequest(self.user.id, friendSecrectCode: code, friendNick: nick, onFriendshipReady: { (error) in
                        if( !error )
                        {
                            Utils.alertMessage( self, title:Utils.localize("notice.title"), message:Utils.localize("notice.addFriendFriendRequestSent"), onAlertClose:{[weak self](action:UIAlertAction) in
                        ...
                        {
                            Utils.alertMessage( self, title:Utils.localize("error.title"), message:Utils.localize("error.addFriendRequest"), onAlertClose:{[weak self] (action:UIAlertAction) in
                                self!.dismissViewControllerAnimated( true, completion: nil);
                            })
                        }
                    })
            ...
            {
                Utils.alertMessage( self, title:Utils.localize("attention.title"), message:Utils.localize("attention.addFriendSendRequestYourSelf"), onAlertClose:nil)
            }
        }
        else
        {
            Utils.alertMessage( self, title:Utils.localize("attention.title"), message:Utils.localize("attention.addFriendNeedTwoFields"), onAlertClose:nil)
        }
    }
