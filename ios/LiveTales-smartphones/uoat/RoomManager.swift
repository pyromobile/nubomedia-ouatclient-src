...
class RoomManager:NSObject, NBMWebRTCPeerDelegate, NBMRoomClientDelegate
{
    ...
    func remotePeers() -> [NBMPeer]
    {
        ...
        if( self.roomClient?.peers != nil )
        {
            ...
            for obj:NSObject in remotePeers
            {
                if( obj is NBMPeer )
                {
    ...
    func webRTCPeer(peer:NBMWebRTCPeer!, didAddStream remoteStream:RTCMediaStream, ofConnection connection:NBMPeerConnection!)
    {
        ...
        if( remotePeer.isEqual( self.localPeer() ) && !self.loopBack )
        {
            ...
            for audioTrack:RTCAudioTrack in remoteStream.audioTracks as! [RTCAudioTrack]
            {
    ...
    //------------------------------------------------------
    // Room events
    func client(client:NBMRoomClient, partecipantJoined peer:NBMPeer)
    {
        print("* client - 12")
        self.delegate!.roomManager( self, peerJoined:peer )
    }
    
    func client(client:NBMRoomClient, partecipantLeft peer:NBMPeer)
    {
        print("* client - 13")
    ...
    func client(client:NBMRoomClient, partecipantEvicted peer:NBMPeer)
    {
        print("* client - 14")
        self.delegate!.roomManager( self, peerEvicted: peer )
    }
    
    func client(client:NBMRoomClient, partecipantPublished peer:NBMPeer)
    {
        print("* client - 15")
        let peerConnection:NBMPeerConnection? = self.connectionOfPeer( peer )
        if( peerConnection == nil && peer.streams.count > 0 )
    ...
    func client(client:NBMRoomClient, partecipantUnpublished peer:NBMPeer)
    {
        print("* client - 16")
        let connectionId:String = self.connectionIdOfPeer( peer )
        self.webRTCPeer?.closeConnectionWithConnectionId( connectionId )
    ...
    func client(client:NBMRoomClient, didReceiveICECandidate candidate:RTCICECandidate, fromPartecipant peer:NBMPeer)
    {
    ...
    func client(client:NBMRoomClient, didReceiveMessage message:String, fromPartecipant peer:NBMPeer)
    {
        print("* client - 18")
