...
class GalleryManagerViewController: UIViewController, UIGestureRecognizerDelegate
{
    ...
    func removePhotos(sender:UIButton)
    {
        ...
        if( !stateOk )
        {
            Utils.alertMessage(self, title:Utils.localize("error.title"), message:Utils.localize("error.galleryRemovePhoto"), onAlertClose: { [unowned self](action) in
                    self.prepareView()
            })
        }
        else
        {
            Utils.alertMessage(self, title:Utils.localize("galleryManagerView.removeAction"), message:Utils.localize("galleryManagerView.photosRemoved"), onAlertClose:{ [unowned self](action) in
                self.prepareView()
            })
        }
    }
    ...
    func savePhotos(sender:UIButton)
    {
        ...
        {
            Utils.alertMessage(self, title:Utils.localize("galleryManagerView.saveAction"), message:Utils.localize("galleryManagerView.photosSaved"), onAlertClose:{ [unowned self](action) in
                self.prepareView()
            })
        }
    }
