...
class TaleModeLobbyViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, LoginSignupDelegate
{
    ...
    func networkStatusChanged(notification:NSNotification)
    {
        ...
        {
            let msg:String = ( self.user.lobby == .Tale ) ? "Internet connection lost!" : "Internet connection lost!\nYou can\'t use free mode!"
            
            Utils.alertMessage(self, title: "Attention", message:msg, onAlertClose: { [unowned self] (action) in
                if( self.user.lobby == .Free )
                {
    ...
    private func prepareFriendList()
    {
        UserModel.loadUserFriends(self.user.id) { [weak self](friends) in
            if( friends == nil )
            {
                ...
                if( NetworkWatcher.internetAvailabe )
                {
                    Utils.alertMessage( self!, title:"Error", message:"Can\'t get friends from database!", onAlertClose:nil )
                }
                else
                {
                    Utils.alertMessage( self!, title:"Attention", message:"You can\'t choose friends\nbecause you haven\'t internet connection!", onAlertClose:nil )
                }
            }
            else
            {
    ...
    private func prepareSuggestLoginView()
    {
        ...
        suggestLabel.text = "To enjoy with your friends you can register!"
        ...
        suggestButton.setTitle( "Register", forState: UIControlState.Normal )
