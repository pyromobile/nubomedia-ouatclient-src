    ...
    */
    ...
    @IBAction func login()
    {
        ...
        else
        {
            ...
            Utils.alertMessage(self, title: "Attention", message: "Internet connection lost!", onAlertClose: { [unowned self] (action) in
                self.dismissViewControllerAnimated( true, completion:{} )
            })
        }
    }
    ...
    @IBAction func signup()
    {
        if( NetworkWatcher.internetAvailabe )
        {
            if( self.signupUserField.text?.isEmpty == false && self.signupPasswordField.text?.isEmpty == false && self.signupPasswordRepeatField.text?.isEmpty == false )
            {
                ...
                {
                    Utils.alertMessage( self, title:"Error!", message:"The passwords not missmatch", onAlertClose:nil )
                }
            }
            else
            {
                Utils.alertMessage( self, title:"Warning!", message:"You must fill three fields.", onAlertClose:nil )
            }
        }
        else
        {
    ...
    private func doLogin( userName:String, password: String )
    {
        UserModel.login( userName, password:Utils.md5(string:password)) { [weak self](isOK, userId, nick, code) in
            ...
            {
                Utils.alertMessage( self!, title:"Error", message:"User or password wrong!", onAlertClose:nil )
            }
        }
    }
