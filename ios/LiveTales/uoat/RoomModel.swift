 ...
 */
    ...
    init(delegate:RoomReadyDelegate,isReadingRoom:Bool,video1View:UIView,video2View:UIView,video3View:UIView,video4View:UIView /*,video5View:UIView*/)
    {
        ...
        //self.mediaConfig!.cameraPosition = NBMCameraPosition.Front
        //self.mediaConfig!.rendererType = .OpenGLES
        self.mediaConfig?.receiverVideoFormat.dimensions.height = Int32((self.video1View?.bounds.height)!)
        self.mediaConfig?.receiverVideoFormat.dimensions.width  = Int32((self.video1View?.bounds.width)!)
        
        print("Dimensions view local -- w:\(self.video1View?.bounds.width) -h:\(self.video1View?.bounds.height)")
        self.roomManager = RoomManager( delegate:self )
    }
    ...
    func joinRoom(userName:String, roomName:String)
    {
        ...
        //Paas: https://a6f5c4869.apps.nubomedia-paas.eu/room (sin marca verde)
        //Paas: https://pa68f43b3.apps.nubomedia-paas.eu/room (con marca verde)
        //-------------------------------------------------------------------------
        //Paas-ouatservice-dev: https://u9b0191b6.apps.nubomedia-paas.eu/room (dev con filtros de VTT y VisualTools)
        
        ConfigModel.getConfig(){ [weak self](config) in
            if( config == nil )
            {
                print("Error get KMS config...")
                if( NetworkWatcher.internetAvailabe )
                {
                    //Utils.alertMessage( self!, title:"Error", message:"Can\'t get KMS config from database!", onAlertClose:nil )
                    print("Error: Can\'t get KMS config from database!")
                    self!.delegate?.onError( ServerResponseCode.DataBaseConfigFail )
                }
                else
                {
                    //Utils.alertMessage( self!, title:"Attention", message:"You can\'t get KMS config\nbecause you haven\'t internet connection!", onAlertClose:nil )
                    print("Error: You can\'t get KMS config because you haven\'t internet connection!")
                    self!.delegate?.onError( ServerResponseCode.NoInternetConnection )
                }
            }
            else
            {
                let roomURL:NSURL = NSURL( string: config!["kms_service_url"]! )!
                self!.room = NBMRoom( username:userName, roomName:roomName, roomURL:roomURL, dataChannels:true )
    ...
    func roomManager(broker:RoomManager, didAddLocalStream localStream:RTCMediaStream)
    {
        ...
        self.video1View!.transform = CGAffineTransformMakeScale( -1, 1 )
    ...
    func roomManager(broker:RoomManager, roomJoined error:NSError?)
    {
        ...
        if( error != nil )
        {
            ...
            let codeError:Int = error?.userInfo["code"] as! Int
            self.delegate?.onError( codeError );
        }
        else
        {
    ...
    func roomManager(broker:RoomManager, didFailWithError error:NSError)
    {
        ...
        self.delegate?.onError( ServerResponseCode.ServerNotWorking )
    }
